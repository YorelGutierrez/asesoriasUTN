...
public function store(Request $request)
{
    ...
    /*public function store(Request $request)
    {
        ...
        try {
            ...
            $fecha_inicio = $request->fecha . ' ' . ($request->hora_inicio ?? '00:00:00');
            $fecha_fin = $request->fecha . ' ' . ($request->hora_fin ?? '00:00:00');
            ...
            $sesion = sesiones_asesoria::create([
                ...
                'estado' => 'programada',
                'observaciones' => 'Tema: ' . $request->tema . ' | Carrera: ' . carreras::find($request->carrera_id)->nombre . ' | Materia: ' . materias::find($request->materia_id)->nombre,
            ]);

            // Asignar los alumnos a la sesión
            foreach ($request->alumnos as $alumnoId) {
                DB::table('sesion_alumno')->insert([
                    'sesion_id' => $sesion->id,
                    'alumno_id' => $alumnoId,
                    'created_at' => now(),
                    'updated_at' => now(),
            ...
            // Obtener datos de alumnos para el Word y PDF
            $alumnosData = alumnos::with(['user', 'grupo'])->whereIn('id', $request->alumnos)->get();

            // ========== 1. GENERAR Y GUARDAR WORD ==========
            $phpWord = new PhpWord();
            $section = $phpWord->addSection();
            ...
            // Información general
            $section->addText('INFORMACIÓN GENERAL', ['bold' => true]);
            $section->addText('Carrera: ' . (carreras::find($request->carrera_id)->nombre ?? 'No especificada'));
            $section->addText('Tipo de asesoría: ' . ucfirst($request->tipo_asesoria));
            $section->addText('Asignatura: ' . (materias::find($request->materia_id)->nombre ?? 'No especificada'));
            $section->addText('Tema: ' . $request->tema);
            $section->addText('Fecha: ' . $request->fecha);
            $section->addText('Horario: ' . ($request->hora_inicio ?? '') . ' - ' . ($request->hora_fin ?? ''));
            $section->addTextBreak(1);
            ...
            foreach ($alumnosData as $index => $alumno) {
                $table->addRow();
                $table->addCell(500)->addText($index + 1);
                $table->addCell(4000)->addText($alumno->user->nombres . ' ' . $alumno->user->apellido_paterno . ' ' . $alumno->user->apellido_materno);
                $table->addCell(1500)->addText($alumno->grupo ? $alumno->grupo->nombre : 'N/A');
                $table->addCell(2000)->addText('_________________');
            }
            
            // Guardar Word
            $nombreArchivo = 'asesoria_' . $sesion->id . '_' . date('Ymd_His') . '.docx';
            $ruta = 'asesorias/' . $nombreArchivo;
            ...
            // Guardar registro en archivos_asesoria
            archivos_asesoria::create([
                'id_referencia' => $sesion->id,
                'tipo_referencia' => 'sesion',
                'nombre_archivo' => $nombreArchivo,
                'ruta' => $ruta,
                'subido_por' => auth()->user()->id,
            ...
            $data = [
                'carrera_nombre' => carreras::find($request->carrera_id)->nombre ?? 'No especificada',
                'materia_nombre' => materias::find($request->materia_id)->nombre ?? 'No especificada',
                'tipo_asesoria' => $request->tipo_asesoria,
                'tema' => $request->tema,
                'fecha' => $request->fecha,
                'hora_inicio' => $request->hora_inicio,
            ...
            // Descargar PDF
            return $pdf->download('asesoria_' . $sesion->id . '_' . date('Ymd_His') . '.pdf');

        } catch (\Exception $e) {
            DB::rollBack();
            return redirect()->back()->with('error', 'Error: ' . $e->getMessage() . ' | Línea: ' . $e->getLine())->withInput();
        }
    }

    // GENERAR PDF (NO guarda en BD)
